...
*/
class spaceInterpMulti : public Operator<SEP::float3DReg, SEP::float3DReg> {

	private:

		/* Spatial interpolation */
		//std::shared_ptr<float3DReg> _elasticPar; /** for dimension checking*/
	...
	public:
		...
		spaceInterpMulti(const std::shared_ptr<float1DReg> zCoord, const std::shared_ptr<float1DReg> xCoord, const std::shared_ptr<SEP::hypercube> elasticParamHyper, int &nt, std::string interpMethod, int nFilt);
		...
		// FWD / ADJ
		void forward(const bool add, const std::shared_ptr<float3DReg> signalReg, std::shared_ptr<float3DReg> signalIrreg) const;
		void adjoint(const bool add, std::shared_ptr<float3DReg> signalReg, const std::shared_ptr<float3DReg> signalIrreg) const;
