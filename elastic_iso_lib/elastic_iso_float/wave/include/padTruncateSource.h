...
*/
class padTruncateSource : public Operator<float3DReg, float4DReg> {
    ...
    */
		padTruncateSource(const std::shared_ptr<float3DReg> model, const std::shared_ptr<float4DReg> data, std::vector<int> gridPointIndexUnique);
