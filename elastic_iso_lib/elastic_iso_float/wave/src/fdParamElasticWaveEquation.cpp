...
fdParamElasticWaveEquation::fdParamElasticWaveEquation(const std::shared_ptr<float3DReg> elasticParam, const std::shared_ptr<paramObj> par) {
	...
	_rhox = new float[_nz * _nx * sizeof(float)]; // Precomputed scaling dtw / rho_x
	_rhoz = new float[_nz * _nx * sizeof(float)]; // Precomputed scaling dtw / rho_z
	_lamb2Mu = new float[_nz * _nx * sizeof(float)]; // Precomputed scaling (lambda + 2*mu) * dtw
	_lamb = new float[_nz * _nx * sizeof(float)]; // Precomputed scaling lambda * dtw
	_muxz = new float[_nz * _nx * sizeof(float)]; // Precomputed scaling mu_xz * dtw

	//initialize 2d slices
	_rhoxReg = std::make_shared<float2DReg>(_elasticParam->getHyper()->getAxis(1), _elasticParam->getHyper()->getAxis(2));
	_rhozReg = std::make_shared<float2DReg>(_elasticParam->getHyper()->getAxis(1), _elasticParam->getHyper()->getAxis(2));
	std::shared_ptr<float2DReg> _temp(new float2DReg(_elasticParam->getHyper()->getAxis(1), _elasticParam->getHyper()->getAxis(2)));
	_lambReg = std::make_shared<float2DReg>(_elasticParam->getHyper()->getAxis(1), _elasticParam->getHyper()->getAxis(2));
	_lamb2MuReg = std::make_shared<float2DReg>(_elasticParam->getHyper()->getAxis(1), _elasticParam->getHyper()->getAxis(2));
	_muxzReg = std::make_shared<float2DReg>(_elasticParam->getHyper()->getAxis(1), _elasticParam->getHyper()->getAxis(2));

	//slice _elasticParam into 2d
	std::memcpy( _temp->getVals(), _elasticParam->getVals(), _nx*_nz*sizeof(float) );
	std::memcpy( _muxzReg->getVals(), _elasticParam->getVals()+2*_nx*_nz, _nx*_nz*sizeof(float) );
...
// bool fdParamElasticWaveEquation::checkFdStability(float CourantMax){
...
// }
//
// bool fdParamElasticWaveEquation::checkFdDispersion(float dispersionRatioMin){
...
bool fdParamElasticWaveEquation::checkParfileConsistencyTime(const std::shared_ptr<float3DReg> seismicTraces, int timeAxisIndex,  std::string fileToCheck) const {
...
bool fdParamElasticWaveEquation::checkParfileConsistencySpace(const std::shared_ptr<float3DReg> model) const {
...
// bool fdParamElastic::checkParfileConsistencySpace(const std::shared_ptr<float3DReg> modelExt) const {
