...
padTruncateSource::padTruncateSource(const std::shared_ptr<float3DReg> model, const std::shared_ptr<float4DReg> data,std::vector<int> gridPointIndexUnique){
...
*/
void padTruncateSource::forward(const bool add, const std::shared_ptr<float3DReg> model, std::shared_ptr<float4DReg> data) const{
...
*/
void padTruncateSource::adjoint(const bool add, std::shared_ptr<float3DReg> model, const std::shared_ptr<float4DReg> data) const{
  assert(checkDomainRange(model,data));
