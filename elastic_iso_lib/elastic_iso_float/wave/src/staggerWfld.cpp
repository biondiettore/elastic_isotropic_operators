...
void staggerWfld::adjoint(const bool add, std::shared_ptr<float4DReg> model,  const std::shared_ptr<float4DReg> data) const {
  ...
  for(int it = 0; it < _nt; it++){
    for(int iz = 0; iz < _nz-1; iz++){
      float pointToSpread_x = (*data->_mat)[it][0][_nx-1][iz];
      (*model->_mat)[it][0][_nx-1][iz] += pointToSpread_x;

      float pointToSpread_z = 0.5 * (*data->_mat)[it][1][_nx-1][iz];
      (*model->_mat)[it][1][_nx-1][iz] += pointToSpread_z;
      (*model->_mat)[it][1][_nx-1][iz+1] += pointToSpread_z;

      float pointToSpread_xz = 0.5 * (*data->_mat)[it][4][_nx-1][iz];
  ...
  for(int it = 0; it < _nt; it++){
    for(int ix = 0; ix < _nx-1; ix++){
      float pointToSpread_x = 0.5 * (*data->_mat)[it][0][ix][_nz-1];
      (*model->_mat)[it][0][ix][_nz-1] += pointToSpread_x;
      (*model->_mat)[it][0][ix+1][_nz-1] += pointToSpread_x;

      float pointToSpread_z = (*data->_mat)[it][1][ix][_nz-1];
      (*model->_mat)[it][1][ix][_nz-1] += pointToSpread_z;

      float pointToSpread_xz = 0.5 * (*data->_mat)[it][4][ix][_nz-1];
