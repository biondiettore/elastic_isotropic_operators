...
void initWaveEquationElasticGpu(float dz, float dx, int nz, int nx, int nts, float dts, int minPad, int blockSize, int nGpu, int iGpuId, int iGpuAlloc){
  ...
  // Host variables
	...
	if (iGpuId == iGpuAlloc) {
		...
		dev_rhox = new float*[nGpu];
		dev_rhoz = new float*[nGpu];
		dev_lamb2Mu = new float*[nGpu];
		dev_lamb = new float*[nGpu];
		dev_muxz = new float*[nGpu];
  }
...
void allocateWaveEquationElasticGpu(float *rhox, float *rhoz, float *lamb2Mu, float *lamb, float *muxz, int iGpu, int iGpuId, int firstTimeSample, int lastTimeSample){
  ...
  cudaSetDevice(iGpuId);
	...
	cuda_call(cudaMalloc((void**) &dev_rhox[iGpu], host_nz*host_nx*sizeof(float)));
  cuda_call(cudaMalloc((void**) &dev_rhoz[iGpu], host_nz*host_nx*sizeof(float)));
  cuda_call(cudaMalloc((void**) &dev_lamb2Mu[iGpu], host_nz*host_nx*sizeof(float)));
  cuda_call(cudaMalloc((void**) &dev_lamb[iGpu], host_nz*host_nx*sizeof(float)));
  cuda_call(cudaMalloc((void**) &dev_muxz[iGpu], host_nz*host_nx*sizeof(float)));

  // Copy scaled elastic parameters to device
	cuda_call(cudaMemcpy(dev_rhox[iGpu], rhox, host_nz*host_nx*sizeof(float), cudaMemcpyHostToDevice));
  cuda_call(cudaMemcpy(dev_rhoz[iGpu], rhoz, host_nz*host_nx*sizeof(float), cudaMemcpyHostToDevice));
  cuda_call(cudaMemcpy(dev_lamb2Mu[iGpu], lamb2Mu, host_nz*host_nx*sizeof(float), cudaMemcpyHostToDevice));
  cuda_call(cudaMemcpy(dev_lamb[iGpu], lamb, host_nz*host_nx*sizeof(float), cudaMemcpyHostToDevice));
  cuda_call(cudaMemcpy(dev_muxz[iGpu], muxz, host_nz*host_nx*sizeof(float), cudaMemcpyHostToDevice));

	// Allocate wavefields on device
	cuda_call(cudaMalloc((void**) &dev_p0[iGpu], (lastTimeSample-firstTimeSample+1)*host_nz*host_nx*host_nw*sizeof(float)));
  cuda_call(cudaMalloc((void**) &dev_p1[iGpu], (lastTimeSample-firstTimeSample+1)*host_nz*host_nx*host_nw*sizeof(float)));
...
void waveEquationElasticFwdGpu(float *model,float *data, int iGpu, int iGpuId, int firstTimeSample, int lastTimeSample){
  ...
  cudaSetDevice(iGpuId);
	...
	//copy model to gpu
	cuda_call(cudaMemcpy(dev_p1[iGpu], model+firstTimeSample*host_nz*host_nx*host_nw, (lastTimeSample-firstTimeSample+1)*host_nz*host_nx*host_nw*sizeof(float), cudaMemcpyHostToDevice));

	//copy data to gpu
	cuda_call(cudaMemcpy(dev_p0[iGpu], data+firstTimeSample*host_nz*host_nx*host_nw, (lastTimeSample-firstTimeSample+1)*host_nz*host_nx*host_nw*sizeof(float), cudaMemcpyHostToDevice));
	...
	if(firstTimeSample==0 && lastTimeSample==host_nts-1) {
		...
		cuda_call(cudaMemcpy(data, dev_p0[iGpu], host_nts*host_nz*host_nx*host_nw*sizeof(float), cudaMemcpyDeviceToHost));
	}
	else if(firstTimeSample==0){
		//exclude last sample | include first sample in block
		cuda_call(cudaMemcpy(data, dev_p0[iGpu], (lastTimeSample-firstTimeSample)*host_nz*host_nx*host_nw*sizeof(float), cudaMemcpyDeviceToHost));
	}
	else if(lastTimeSample==host_nts-1){
		//exclude first sample | include last sample in block
		cuda_call(cudaMemcpy(data+(firstTimeSample+1)*host_nz*host_nx*host_nw, dev_p0[iGpu]+1*host_nz*host_nx*host_nw, (lastTimeSample-firstTimeSample)*host_nz*host_nx*host_nw*sizeof(float), cudaMemcpyDeviceToHost));
	}
	else{
		//exclude first and last sample in block
		cuda_call(cudaMemcpy(data+(firstTimeSample+1)*host_nz*host_nx*host_nw, dev_p0[iGpu]+1*host_nz*host_nx*host_nw, (lastTimeSample-firstTimeSample-1)*host_nz*host_nx*host_nw*sizeof(float), cudaMemcpyDeviceToHost));
	}
}
void waveEquationElasticAdjGpu(float *model,float *data, int iGpu, int iGpuId, int firstTimeSample, int lastTimeSample){
  ...
  cudaSetDevice(iGpuId);
	...
	//copy data to gpu
	cuda_call(cudaMemcpy(dev_p1[iGpu], data+firstTimeSample*host_nz*host_nx*host_nw, (lastTimeSample-firstTimeSample+1)*host_nz*host_nx*host_nw*sizeof(float), cudaMemcpyHostToDevice));

	//copy model to gpu
	cuda_call(cudaMemcpy(dev_p0[iGpu], model+firstTimeSample*host_nz*host_nx*host_nw, (lastTimeSample-firstTimeSample+1)*host_nz*host_nx*host_nw*sizeof(float), cudaMemcpyHostToDevice));
	...
	if(firstTimeSample==0 && lastTimeSample==host_nts-1) {
		//include first and last sample in block
		cuda_call(cudaMemcpy(model, dev_p0[iGpu], host_nts*host_nz*host_nx*host_nw*sizeof(float), cudaMemcpyDeviceToHost));
	}
	else if(firstTimeSample==0){
		//exclude last sample | include first sample in block
		cuda_call(cudaMemcpy(model, dev_p0[iGpu], (lastTimeSample-firstTimeSample)*host_nz*host_nx*host_nw*sizeof(float), cudaMemcpyDeviceToHost));
	}
	else if(lastTimeSample==host_nts-1){
		//exclude first sample | include last sample in block
		cuda_call(cudaMemcpy(model+(firstTimeSample+1)*host_nz*host_nx*host_nw, dev_p0[iGpu]+1*host_nz*host_nx*host_nw, (lastTimeSample-firstTimeSample)*host_nz*host_nx*host_nw*sizeof(float), cudaMemcpyDeviceToHost));
	}
	else{
		//exclude first and last sample in block
		cuda_call(cudaMemcpy(model+(firstTimeSample+1)*host_nz*host_nx*host_nw, dev_p0[iGpu]+1*host_nz*host_nx*host_nw, (lastTimeSample-firstTimeSample-1)*host_nz*host_nx*host_nw*sizeof(float), cudaMemcpyDeviceToHost));
	}
}
