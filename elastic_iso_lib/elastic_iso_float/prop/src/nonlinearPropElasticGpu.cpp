...
bool nonlinearPropElasticGpu::checkParfileConsistency(const std::shared_ptr<SEP::float3DReg> model, const std::shared_ptr<SEP::float3DReg> data) const{
...
void nonlinearPropElasticGpu::forward(const bool add, const std::shared_ptr<float3DReg> model, std::shared_ptr<float3DReg> data) const {

  /* Allocation */
  std::shared_ptr<float2DReg> modelTemp_vx(new float2DReg(_fdParamElastic->_nts, _nSourcesIrregXGrid));
  std::shared_ptr<float2DReg> modelTemp_vz(new float2DReg(_fdParamElastic->_nts, _nSourcesIrregZGrid));
  std::shared_ptr<float2DReg> modelTemp_sigmaxx(new float2DReg(_fdParamElastic->_nts, _nSourcesIrregCenterGrid));
  std::shared_ptr<float2DReg> modelTemp_sigmazz(new float2DReg(_fdParamElastic->_nts, _nSourcesIrregCenterGrid));
  std::shared_ptr<float2DReg> modelTemp_sigmaxz(new float2DReg(_fdParamElastic->_nts, _nSourcesIrregXZGrid));
  ...
  // std::shared_ptr<float2DReg> dataRegDtw(new float2DReg(_fdParamElastic->_ntw, _nReceiversReg));
  ...
  std::shared_ptr<float2DReg> dataTemp_vx(new float2DReg(_fdParamElastic->_nts, _nReceiversIrregXGrid));
  std::shared_ptr<float2DReg> dataTemp_vz(new float2DReg(_fdParamElastic->_nts, _nReceiversIrregZGrid));
  std::shared_ptr<float2DReg> dataTemp_sigmaxx(new float2DReg(_fdParamElastic->_nts, _nReceiversIrregCenterGrid));
  std::shared_ptr<float2DReg> dataTemp_sigmazz(new float2DReg(_fdParamElastic->_nts, _nReceiversIrregCenterGrid));
  std::shared_ptr<float2DReg> dataTemp_sigmaxz(new float2DReg(_fdParamElastic->_nts, _nReceiversIrregXZGrid));
...
void nonlinearPropElasticGpu::adjoint(const bool add, std::shared_ptr<float3DReg> model, const std::shared_ptr<float3DReg> data) const {
	...
	if (_saveWavefield == 0) {
		propShotsElasticAdjGpu(modelRegDtw_vx->getVals(),
				...
				std::cerr << "propShotsElasticAdjGpuWavefield" << '\n';
