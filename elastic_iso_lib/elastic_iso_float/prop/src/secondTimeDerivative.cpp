...
secondTimeDerivative::secondTimeDerivative(int nt, float dt) {
	_nt = nt;
	_dt2 = 1.0 / (dt*dt);	
}

void secondTimeDerivative::forward(const bool add, const std::shared_ptr<float2DReg> model, std::shared_ptr<float2DReg> data) const
{
...
void secondTimeDerivative::adjoint(const bool add, std::shared_ptr<float2DReg> model, const std::shared_ptr<float2DReg> data) const{
