...
// Forward
void nonlinearPropElasticShotsGpu::forward(const bool add, const std::shared_ptr<float4DReg> model, std::shared_ptr<float4DReg> data) const{
    ...
    if (model->getHyper()->getAxis(3).n != 5) {throw std::runtime_error("");;}
  ...
  for (int iExp=0; iExp<_nExp; iExp++){
    ...
    if(constantSrcSignal == 1) {
      ...
      memcpy(modelSlicesVector[iGpu]->getVals(), model->getVals(), modelsize*sizeof(float));
    } else {
			long long modelsize = hyperModelSlices->getAxis(1).n*hyperModelSlices->getAxis(2).n;
			modelsize *= hyperModelSlices->getAxis(3).n;
      memcpy(modelSlicesVector[iGpu]->getVals(), &(model->getVals()[iExp*modelsize]), sizeof(float)*modelsize);
    }
...
void nonlinearPropElasticShotsGpu::forwardWavefield(const bool add, const std::shared_ptr<float4DReg> model, std::shared_ptr<float4DReg> data){
...
// Adjoint
void nonlinearPropElasticShotsGpu::adjoint(const bool add, std::shared_ptr<SEP::float4DReg> model, const std::shared_ptr<SEP::float4DReg> data) const{
