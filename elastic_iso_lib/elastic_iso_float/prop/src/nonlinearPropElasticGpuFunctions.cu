...
void modelAllocateGpu(int nSourcesRegCenterGrid, int nSourcesRegXGrid, int nSourcesRegZGrid, int nSourcesRegXZGrid, int iGpu){
		cuda_call(cudaMalloc((void**) &dev_modelRegDtw_vx[iGpu], nSourcesRegXGrid*host_nts*sizeof(float))); // Allocate input on device
		cuda_call(cudaMalloc((void**) &dev_modelRegDtw_vz[iGpu], nSourcesRegZGrid*host_nts*sizeof(float))); // Allocate input on device
		cuda_call(cudaMalloc((void**) &dev_modelRegDtw_sigmaxx[iGpu], nSourcesRegCenterGrid*host_nts*sizeof(float))); // Allocate input on device
		cuda_call(cudaMalloc((void**) &dev_modelRegDtw_sigmazz[iGpu], nSourcesRegCenterGrid*host_nts*sizeof(float))); // Allocate input on device
		cuda_call(cudaMalloc((void**) &dev_modelRegDtw_sigmaxz[iGpu], nSourcesRegXZGrid*host_nts*sizeof(float))); // Allocate input on device
}
...
void modelCopyToGpu(float *modelRegDtw_vx, float *modelRegDtw_vz, float *modelRegDtw_sigmaxx, float *modelRegDtw_sigmazz, float *modelRegDtw_sigmaxz, int nSourcesRegCenterGrid, int nSourcesRegXGrid, int nSourcesRegZGrid, int nSourcesRegXZGrid, int iGpu){
		cuda_call(cudaMemcpy(dev_modelRegDtw_vx[iGpu], modelRegDtw_vx, nSourcesRegXGrid*host_nts*sizeof(float), cudaMemcpyHostToDevice)); // Copy input signals on device
		cuda_call(cudaMemcpy(dev_modelRegDtw_vz[iGpu], modelRegDtw_vz, nSourcesRegZGrid*host_nts*sizeof(float), cudaMemcpyHostToDevice)); // Copy input signals on device
		cuda_call(cudaMemcpy(dev_modelRegDtw_sigmaxx[iGpu], modelRegDtw_sigmaxx, nSourcesRegCenterGrid*host_nts*sizeof(float), cudaMemcpyHostToDevice)); // Copy input signals on device
		cuda_call(cudaMemcpy(dev_modelRegDtw_sigmazz[iGpu], modelRegDtw_sigmazz, nSourcesRegCenterGrid*host_nts*sizeof(float), cudaMemcpyHostToDevice)); // Copy input signals on device
		cuda_call(cudaMemcpy(dev_modelRegDtw_sigmaxz[iGpu], modelRegDtw_sigmaxz, nSourcesRegXZGrid*host_nts*sizeof(float), cudaMemcpyHostToDevice)); // Copy input signals on device
}
...
void modelInitializeOnGpu(int nSourcesRegCenterGrid, int nSourcesRegXGrid, int nSourcesRegZGrid, int nSourcesRegXZGrid, int iGpu){
		cuda_call(cudaMemset(dev_modelRegDtw_vx[iGpu], 0, nSourcesRegXGrid*host_nts*sizeof(float))); // Copy input signals on device
		cuda_call(cudaMemset(dev_modelRegDtw_vz[iGpu], 0, nSourcesRegZGrid*host_nts*sizeof(float))); // Copy input signals on device
		cuda_call(cudaMemset(dev_modelRegDtw_sigmaxx[iGpu], 0, nSourcesRegCenterGrid*host_nts*sizeof(float))); // Copy input signals on device
		cuda_call(cudaMemset(dev_modelRegDtw_sigmazz[iGpu], 0, nSourcesRegCenterGrid*host_nts*sizeof(float))); // Copy input signals on device
		cuda_call(cudaMemset(dev_modelRegDtw_sigmaxz[iGpu], 0, nSourcesRegXZGrid*host_nts*sizeof(float))); // Copy input signals on device
}
//allocate model on device
...
void propShotsElasticFwdGpuWavefield(float *modelRegDtw_vx, float *modelRegDtw_vz, float *modelRegDtw_sigmaxx, float *modelRegDtw_sigmazz, float *modelRegDtw_sigmaxz, float *dataRegDts_vx, float *dataRegDts_vz, float *dataRegDts_sigmaxx, float *dataRegDts_sigmazz, float *dataRegDts_sigmaxz, int *sourcesPositionRegCenterGrid, int nSourcesRegCenterGrid, int *sourcesPositionRegXGrid, int nSourcesRegXGrid, int *sourcesPositionRegZGrid, int nSourcesRegZGrid, int *sourcesPositionRegXZGrid, int nSourcesRegXZGrid, int *receiversPositionRegCenterGrid, int nReceiversRegCenterGrid, int *receiversPositionRegXGrid, int nReceiversRegXGrid, int *receiversPositionRegZGrid, int nReceiversRegZGrid, int *receiversPositionRegXZGrid, int nReceiversRegXZGrid, float* wavefieldDts, int iGpu, int iGpuId, int surfaceCondition){
...
void propShotsElasticAdjGpu(float *modelRegDtw_vx, float *modelRegDtw_vz, float *modelRegDtw_sigmaxx, float *modelRegDtw_sigmazz, float *modelRegDtw_sigmaxz, float *dataRegDts_vx, float *dataRegDts_vz, float *dataRegDts_sigmaxx, float *dataRegDts_sigmazz, float *dataRegDts_sigmaxz, int *sourcesPositionRegCenterGrid, int nSourcesRegCenterGrid, int *sourcesPositionRegXGrid, int nSourcesRegXGrid, int *sourcesPositionRegZGrid, int nSourcesRegZGrid, int *sourcesPositionRegXZGrid, int nSourcesRegXZGrid, int *receiversPositionRegCenterGrid, int nReceiversRegCenterGrid, int *receiversPositionRegXGrid, int nReceiversRegXGrid, int *receiversPositionRegZGrid, int nReceiversRegZGrid, int *receiversPositionRegXZGrid, int nReceiversRegXZGrid, int iGpu, int iGpuId, int surfaceCondition) {
		...
		cuda_call(cudaMemcpy(modelRegDtw_vx, dev_modelRegDtw_vx[iGpu], nSourcesRegXGrid*host_nts*sizeof(float), cudaMemcpyDeviceToHost));
		cuda_call(cudaMemcpy(modelRegDtw_vz, dev_modelRegDtw_vz[iGpu], nSourcesRegZGrid*host_nts*sizeof(float), cudaMemcpyDeviceToHost));
		cuda_call(cudaMemcpy(modelRegDtw_sigmaxx, dev_modelRegDtw_sigmaxx[iGpu], nSourcesRegCenterGrid*host_nts*sizeof(float), cudaMemcpyDeviceToHost));
		cuda_call(cudaMemcpy(modelRegDtw_sigmazz, dev_modelRegDtw_sigmazz[iGpu], nSourcesRegCenterGrid*host_nts*sizeof(float), cudaMemcpyDeviceToHost));
		cuda_call(cudaMemcpy(modelRegDtw_sigmaxz, dev_modelRegDtw_sigmaxz[iGpu], nSourcesRegXZGrid*host_nts*sizeof(float), cudaMemcpyDeviceToHost));
...
void propShotsElasticAdjGpuWavefieldStreams(float *modelRegDtw_vx, float *modelRegDtw_vz, float *modelRegDtw_sigmaxx, float *modelRegDtw_sigmazz, float *modelRegDtw_sigmaxz, float *dataRegDts_vx, float *dataRegDts_vz, float *dataRegDts_sigmaxx, float *dataRegDts_sigmazz, float *dataRegDts_sigmaxz, int *sourcesPositionRegCenterGrid, int nSourcesRegCenterGrid, int *sourcesPositionRegXGrid, int nSourcesRegXGrid, int *sourcesPositionRegZGrid, int nSourcesRegZGrid, int *sourcesPositionRegXZGrid, int nSourcesRegXZGrid, int *receiversPositionRegCenterGrid, int nReceiversRegCenterGrid, int *receiversPositionRegXGrid, int nReceiversRegXGrid, int *receiversPositionRegZGrid, int nReceiversRegZGrid, int *receiversPositionRegXZGrid, int nReceiversRegXZGrid, float* wavefieldDts, int iGpu, int iGpuId){
		...
		//wavefield allocation and setup
		cuda_call(cudaMalloc((void**) &dev_wavefieldDts_left[iGpu], time_slice*sizeof(float))); // Allocate on device
		cuda_call(cudaMalloc((void**) &dev_wavefieldDts_right[iGpu], time_slice*sizeof(float))); // Allocate on device
		cuda_call(cudaMalloc((void**) &dev_pStream[iGpu], time_slice*sizeof(float))); // Allocate on device
		cuda_call(cudaHostAlloc((void**) &pin_wavefieldSlice[iGpu], time_slice*sizeof(float), cudaHostAllocDefault)); //Pinned memory on the Host
		//Initialize wavefield slides
		cuda_call(cudaMemset(dev_wavefieldDts_left[iGpu], 0, time_slice*sizeof(float))); // Initialize left slide on device
		cuda_call(cudaMemset(dev_wavefieldDts_right[iGpu], 0, time_slice*sizeof(float))); // Initialize right slide on device
		cuda_call(cudaMemset(dev_pStream[iGpu], 0, time_slice*sizeof(float))); // Initialize temporary slice on device
		cudaMemset(pin_wavefieldSlice[iGpu], 0, time_slice*sizeof(float)); // Initialize pinned memory
		...
		for (int its = host_nts-2; its > -1; its--){
				...
				cuda_call(cudaMemcpyAsync(dev_pStream[iGpu], dev_wavefieldDts_right[iGpu], time_slice*sizeof(float), cudaMemcpyDeviceToDevice, compStream[iGpu]));
				...
				// Asynchronous transfer of pStream => pin [its] [transfer]
				// Launch the transfer while we compute the next coarse time sample
				cuda_call(cudaMemcpyAsync(pin_wavefieldSlice[iGpu], dev_pStream[iGpu], time_slice*sizeof(float), cudaMemcpyDeviceToHost, transferStream[iGpu]));
				...
				dev_wavefieldDts_right[iGpu] = dev_temp1[iGpu];
				dev_temp1[iGpu] = NULL;
				cuda_call(cudaMemsetAsync(dev_wavefieldDts_left[iGpu], 0, time_slice*sizeof(float), compStream[iGpu])); // Reinitialize dev_wavefieldDts_right to zero

		}

		// Note: At that point, dev_wavefieldDts_left contains the value of the wavefield at nts-1 (last time sample) and the wavefield only has values up to nts-3
		cuda_call(cudaStreamSynchronize(transferStream[iGpu]));	// [transfer]
		std::memcpy(wavefieldDts+time_slice,pin_wavefieldSlice[iGpu], time_slice*sizeof(float)); // Copy pinned array to wavefield array [its=1] [host]
		cuda_call(cudaStreamSynchronize(compStream[iGpu]));	// Make sure dev_pLeft => dev_pStream is done
		cuda_call(cudaMemcpyAsync(wavefieldDts, dev_wavefieldDts_right[iGpu], time_slice*sizeof(float), cudaMemcpyDeviceToHost, transferStream[iGpu])); // Copy pinned array to wavefield array for the last sample [its=0] [host]
		cuda_call(cudaStreamSynchronize(transferStream[iGpu]));
