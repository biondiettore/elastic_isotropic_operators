...
BornElasticShotsGpu::BornElasticShotsGpu(std::shared_ptr<SEP::float3DReg> elasticParam,
...
void BornElasticShotsGpu::forward(const bool add, const std::shared_ptr<float3DReg> model, std::shared_ptr<float4DReg> data) const{
    ...
    std::shared_ptr<float2DReg> temp(new float2DReg(_elasticParam->getHyper()->getAxis(1), _elasticParam->getHyper()->getAxis(2)));
    std::shared_ptr<float2DReg> temp1(new float2DReg(_elasticParam->getHyper()->getAxis(1), _elasticParam->getHyper()->getAxis(2)));
    std::shared_ptr<SEP::float3DReg> modelSlice(new SEP::float3DReg(hyperModelSlices));
    ...
    //D_RHOX
    std::memcpy( temp->getVals(), model->getVals(), nx*nz*sizeof(float) );
    staggerXop->adjoint(false, temp1, temp);
    std::memcpy( modelSlice->getVals(), temp1->getVals(), nx*nz*sizeof(float) );
    //D_RHOZ
    staggerZop->adjoint(false, temp1, temp);
    std::memcpy( modelSlice->getVals()+nx*nz, temp1->getVals(), nx*nz*sizeof(float) );

    //D_LAME
    std::memcpy( modelSlice->getVals()+2*nx*nz, model->getVals()+nx*nz, nx*nz*sizeof(float) );

    //SHEAR MODULUS PERTURBATION
    //D_MU
    std::memcpy( modelSlice->getVals()+3*nx*nz, model->getVals()+2*nx*nz, nx*nz*sizeof(float) );
    //D_MUXZ
    std::memcpy( temp->getVals(), model->getVals()+2*nx*nz, nx*nz*sizeof(float) );
    staggerXop->adjoint(false, temp1, temp);
    staggerZop->adjoint(false, temp, temp1);
    std::memcpy( modelSlice->getVals()+4*nx*nz, temp->getVals(), nx*nz*sizeof(float) );
...
void BornElasticShotsGpu::adjoint(const bool add, const std::shared_ptr<float3DReg> model, std::shared_ptr<float4DReg> data) const{
    ...
    //DENSITY PERTURBATION
    //D_RHOX
    std::memcpy( temp1->getVals(), model->getVals(), nx*nz*sizeof(float) );
		std::memcpy( temp->getVals(), modelSlicesVector[0]->getVals(), nx*nz*sizeof(float) );
		staggerXop->forward(true, temp, temp1);
    //D_RHOZ
    std::memcpy( temp->getVals(), modelSlicesVector[0]->getVals()+nx*nz, nx*nz*sizeof(float) );
		staggerZop->forward(true, temp, temp1);
    std::memcpy( model->getVals(), temp1->getVals(), nx*nz*sizeof(float) );

		//UNSTAGGERING D_MUXZ
    std::memcpy( temp->getVals(), modelSlicesVector[0]->getVals()+4*nx*nz, nx*nz*sizeof(float) );
