...
bool BornElasticGpu::checkParfileConsistency(const std::shared_ptr<SEP::float3DReg> model, const std::shared_ptr<SEP::float3DReg> data) const{
...
void BornElasticGpu::forward(const bool add, const std::shared_ptr<float3DReg> model, std::shared_ptr<float3DReg> data) const {
	...
	float *drhoz_in = model->getVals()+_fdParamElastic->_nz*_fdParamElastic->_nx;
	float *dlame_in = model->getVals()+2*_fdParamElastic->_nz*_fdParamElastic->_nx;
	float *dmu_in   = model->getVals()+3*_fdParamElastic->_nz*_fdParamElastic->_nx;
	float *dmuxz_in = model->getVals()+4*_fdParamElastic->_nz*_fdParamElastic->_nx;
...
void BornElasticGpu::adjoint(const bool add, const std::shared_ptr<float3DReg> model, std::shared_ptr<float3DReg> data) const {
