...
// }
//
// void staggerWfld::forward(const bool add,const  std::shared_ptr<float4DReg> model,  std::shared_ptr<float4DReg> data) const {
//
//   assert(checkDomainRange(model,data));
//   if(!add) data->scale(0.);
//
...
// }
//
// void staggerWfld::adjoint(const bool add, std::shared_ptr<float4DReg> model,  const std::shared_ptr<float4DReg> data) const {
//
//   assert(checkDomainRange(model,data));
//   if(!add) model->scale(0.);
//
...
//   for(int it = 0; it < _nt; it++){
//     for(int iz = 0; iz < _nz-1; iz++){
//       float pointToSpread_x = (*data->_mat)[0][it][_nx-1][iz];
//       (*model->_mat)[0][it][_nx-1][iz] += pointToSpread_x;
//
//       float pointToSpread_z = 0.5 * (*data->_mat)[1][it][_nx-1][iz];
//       (*model->_mat)[1][it][_nx-1][iz] += pointToSpread_z;
//       (*model->_mat)[1][it][_nx-1][iz+1] += pointToSpread_z;
//
//       float pointToSpread_xz = 0.5 * (*data->_mat)[4][it][_nx-1][iz];
...
//   for(int it = 0; it < _nt; it++){
//     for(int ix = 0; ix < _nx-1; ix++){
//       float pointToSpread_x = 0.5 * (*data->_mat)[0][it][ix][_nz-1];
//       (*model->_mat)[0][it][ix][_nz-1] += pointToSpread_x;
//       (*model->_mat)[0][it][ix+1][_nz-1] += pointToSpread_x;
//
//       float pointToSpread_z = (*data->_mat)[1][it][ix][_nz-1];
//       (*model->_mat)[1][it][ix][_nz-1] += pointToSpread_z;
//
//       float pointToSpread_xz = 0.5 * (*data->_mat)[4][it][ix][_nz-1];
