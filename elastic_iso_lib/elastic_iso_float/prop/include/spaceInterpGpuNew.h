...
class spaceInterpGpu : public Operator<SEP::float2DReg, SEP::float2DReg> {
	...
	public:
		...
		spaceInterpGpu(const std::shared_ptr<float1DReg> zCoord, const std::shared_ptr<float1DReg> xCoord, const std::shared_ptr<SEP::hypercube> elasticParamHyper, int nt, std::string interpMethod="linear", int nFilt=4);
