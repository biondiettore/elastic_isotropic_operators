...
if __name__ == '__main__':
  ...
  if (pyinfo):
    print("-------------------------------------------------------------------")
  if (pyinfo):
    print("------------------ wavefield reconstruction --------------")
  if (pyinfo):
    print(
        "-------------------------------------------------------------------\n")
  ...
  modelDouble, dataDouble, elasticParamDouble, parObject = Elastic_iso_double_we.waveEquationOpInitDouble(
      sys.argv)
  waveEquationElasticOp = Elastic_iso_double_we.waveEquationElasticGpu(
      modelDouble, dataDouble, elasticParamDouble, parObject)
  ...
  # scale prior
  prior.scale(1 / (elasticParamDouble.getHyper().getAxis(1).d *
                   elasticParamDouble.getHyper().getAxis(2).d))
  ...
  dataDouble = SepVector.getSepVector(dataSamplingOp.getRange().getHyper(),
                                      storage="dataDouble")
  ...
  ############################# Regularization ###############################
  epsilon = 0
  invProb = Prblm.ProblemL2LinearReg(modelInit,
                                     dataDouble,
                                     dataSamplingOp,
                                     epsilon,
                                     reg_op=waveEquationElasticOp,
                                     prior_model=prior)
  ...
  outputFloat = SepVector.getSepVector(outputDouble.getHyper(),
                                       storage="dataFloat")
