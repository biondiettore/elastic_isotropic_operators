...
def forcing_term_op_init(args):
  ...
  zCoord, xCoord, centerHyper = SpaceInterpMultiDouble.space_interp_multi_init_source(
      args)
  ...
  spaceInterpMultiOp = SpaceInterpMultiDouble.space_interp_multi(
  ...
  padTruncateSourceOp = PadTruncateSourceDouble.pad_truncate_source(
  ...
  staggerDummyModel = SepVector.getSepVector(padTruncateDummyData.getHyper(),
                                             storage="dataDouble")
  output = SepVector.getSepVector(padTruncateDummyData.getHyper(),
                                  storage="dataDouble")
  ...
  #read in source
  waveletDouble = SepVector.getSepVector(SPK_adj.getDomain().getHyper(),
                                         storage="dataDouble")
  prior = SepVector.getSepVector(SPK_adj.getRange().getHyper(),
                                 storage="dataDouble")
...
def data_extraction_op_init(args):
  ...
  # Interp operator init
  zCoord, xCoord, centerHyper = SpaceInterpMultiDouble.space_interp_multi_init_rec(
      args)
  ...
  nt = parObject.getInt("nts")
  spaceInterpMultiOp = SpaceInterpMultiDouble.space_interp_multi(
      zCoord, xCoord, centerHyper, nt, recInterpMethod, recInterpNumFilters)
  ...
  irregRecAxis = Hypercube.axis(n=spaceInterpMultiOp.getNDeviceIrreg(),
                                o=0.0,
                                d=1)
  ...
  padTruncateDummyModel = SepVector.getSepVector(regRecHyper,
                                                 storage="dataDouble")
  padTruncateDummyData = SepVector.getSepVector(regWfldHyper,
                                                storage="dataDouble")
  recGridPositions = spaceInterpMultiOp.getRegPosUniqueVector()
  padTruncateRecOp = PadTruncateSourceDouble.pad_truncate_source(
      padTruncateDummyModel, padTruncateDummyData, recGridPositions)
  padTruncateRecOp = Op.Transpose(padTruncateRecOp)

  #stagger op
  staggerDummyModel = SepVector.getSepVector(padTruncateDummyData.getHyper(),
                                             storage="dataDouble")
  input = SepVector.getSepVector(padTruncateDummyData.getHyper(),
                                 storage="dataDouble")
