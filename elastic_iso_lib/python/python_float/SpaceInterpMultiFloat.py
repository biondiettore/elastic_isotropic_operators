...
def space_interp_multi_init_rec(args):
	...
	# rec geometry
	nzReceiver=1
	ozReceiver=parObject.getInt("depthReceiver")-1+parObject.getInt("zPadMinus",0)+parObject.getInt("fat")
	dzReceiver=0
	nxReceiver=parObject.getInt("nReceiver")
	oxReceiver=parObject.getInt("oReceiver")-1+parObject.getInt("xPadMinus",0)+parObject.getInt("fat")
...
class space_interp_multi(Op.Operator):
	...
	def __init__(self,zCoord,xCoord,elasticParamHypercube,nt,interpMethod,nFilt):
