...
def create_client(parObject):
  ...
  if hostnames != "noHost" and cluster_args is not None:
    raise ValueError(
        "Only one interface can be used for a client! User provided both SSH and PBS/LSF parameters!"
    )
  ...
  if hostnames != "noHost":
    ...
    cluster_dict = {
        ...
        for elem in cluster_args.split(",")
    }
    ...
    print("Starting jobqueue Dask client using %s workers on %s jobs" %
          (n_wrks, n_jobs))
...
def spreadParObj(client, args, par):
  ...
  for idx, wrkId in enumerate(client.getWorkerIds()):
    ...
    parObject.append(client.getClient().submit(create_parObj,
                                               args1,
                                               workers=[wrkId],
                                               pure=False))
...
def call_spaceInterpGpu(zAxes, xAxes, zCoord, xCoord, GridAxes, nts,
                        sourceInterpMethod, sourceInterpNumFilters, dipole,
                        zDipoleShift, xDipoleShift):
  ...
  obj = spaceInterpGpu(zCoordFloat.getCpp(), xCoordFloat.getCpp(),
                       GridHyper.getCpp(), nts, sourceInterpMethod,
...
def chunkData(dataVecLocal, dataSpaceRemote):
  ...
  dataAxes = client.gather(
      client.map(get_axes, dataSpaceRemote.vecDask,
                 pure=False))  #Getting hypercubes of remote vector chunks
  List_Exps = [axes[-1].n for axes in dataAxes]
  dataNd = dataVecLocal.getNdArray()
  if (np.sum(List_Exps) != dataNd.shape[0]):
    raise ValueError(
        "Number of shot within provide data vector (%s) not consistent with total number of shots from nShot parameter (%s)"
        % (dataNd.shape[0], np.sum(List_Exps)))
  ...
  for idx, wrkId in enumerate(wrkIds):
    ...
    daskD.wait(arrD)
    daskD.wait(
        client.submit(pyDaskVector.copy_from_NdArray,
                      dataVecRemote.vecDask[idx],
                      arrD,
                      workers=[wrkId],
                      pure=False))
...
def buildSourceGeometryDask(parObject, elasticParamHyper, client):
  ...
  if len(List_Exps) != nWrks:
    raise ValueError(
        "Number of workers (#nWrk=%s) not consistent with length of the provided list of experiments (nExp=%s)"
        % (nWrks, List_Exps))
  ...
  #need a hypercube for centerGrid, x shifted, z shifted, and xz shifted grid
  zAxis = Hypercube.axis(n=elasticParamHyper.axes[0].n,
                         o=elasticParamHyper.axes[0].o,
                         d=elasticParamHyper.axes[0].d)
  zAxisShifted = Hypercube.axis(n=elasticParamHyper.axes[0].n,
                                o=elasticParamHyper.axes[0].o -
                                0.5 * elasticParamHyper.axes[0].d,
                                d=elasticParamHyper.axes[0].d)

  xAxis = Hypercube.axis(n=elasticParamHyper.axes[1].n,
                         o=elasticParamHyper.axes[1].o,
                         d=elasticParamHyper.axes[1].d)
  xAxisShifted = Hypercube.axis(n=elasticParamHyper.axes[1].n,
                                o=elasticParamHyper.axes[1].o -
                                0.5 * elasticParamHyper.axes[1].d,
                                d=elasticParamHyper.axes[1].d)

  paramAxis = Hypercube.axis(n=elasticParamHyper.axes[2].n,
                             o=elasticParamHyper.axes[2].o,
                             d=elasticParamHyper.axes[2].d)
  ...
  for idx, nExp in enumerate(List_Exps):
    ...
    for ishot in range(nExp):
      ...
      sourcesVectorCenterGrid[idx].append(client.getClient().submit(
          call_spaceInterpGpu,
          zAxes,
          xAxes,
          zCoord,
          ...
          workers=wrkIds[idx],
          pure=False))
      sourcesVectorXGrid[idx].append(client.getClient().submit(
          call_spaceInterpGpu,
          zAxes,
          xAxes,
          zCoord,
          ...
          workers=wrkIds[idx],
          pure=False))
      sourcesVectorZGrid[idx].append(client.getClient().submit(
          call_spaceInterpGpu,
          zAxes,
          xAxes,
          zCoord,
          ...
          workers=wrkIds[idx],
          pure=False))
      sourcesVectorXZGrid[idx].append(client.getClient().submit(
          call_spaceInterpGpu,
          zAxes,
          xAxes,
          zCoord,
    ...
    daskD.wait(sourcesVectorCenterGrid[idx] + sourcesVectorXGrid[idx] +
               sourcesVectorZGrid[idx] + sourcesVectorXZGrid[idx])
...
def buildReceiversGeometryDask(parObject, elasticParamHyper, client):
  ...
  receiverAxis = [
      Hypercube.axis(n=nxReceiver, o=ox + oxReceiver * dx, d=dxReceiver * dx)
  ] * nWrks
  for iwrk in range(nWrks):
    recVectorCenterGrid[iwrk].append(client.getClient().submit(
        call_spaceInterpGpu, [recAxisVertical], [recAxisHorizontal],
        zCoordFloatNd,
        xCoordFloatNd,
        centerGridAxes,
        nts,
        ...
        workers=wrkIds[iwrk],
        pure=False))
    recVectorXGrid[iwrk].append(client.getClient().submit(call_spaceInterpGpu,
                                                          [recAxisVertical],
                                                          [recAxisHorizontal],
                                                          zCoordFloatNd,
                                                          xCoordFloatNd,
                                                          ...
                                                          workers=wrkIds[iwrk],
                                                          pure=False))
    recVectorZGrid[iwrk].append(client.getClient().submit(call_spaceInterpGpu,
                                                          [recAxisVertical],
                                                          [recAxisHorizontal],
                                                          zCoordFloatNd,
                                                          xCoordFloatNd,
                                                          ...
                                                          workers=wrkIds[iwrk],
                                                          pure=False))
    recVectorXZGrid[iwrk].append(client.getClient().submit(call_spaceInterpGpu,
                                                           [recAxisVertical],
                                                           [recAxisHorizontal],
                                                           zCoordFloatNd,
                                                           xCoordFloatNd,
...
def nonlinearOpInitFloat(args, client=None):
  ...
  if client:
    ...
    elasticParamFloat = pyDaskVector.DaskVector(client,
                                                vectors=[elasticParamFloat] *
                                                nWrks).vecDask

    # Build sources/receivers geometry
    sourcesVectorCenterGrid, sourcesVectorXGrid, sourcesVectorZGrid, sourcesVectorXZGrid, sourceAxis = buildSourceGeometryDask(
        parObject, elasticParamHyper, client)
    recVectorCenterGrid, recVectorXGrid, recVectorZGrid, recVectorXZGrid, receiverAxis = buildReceiversGeometryDask(
        parObject, elasticParamHyper, client)
    ...
    for iwrk in range(nWrks):
      dataHyper = Hypercube.hypercube(
          axes=[timeAxis, receiverAxis[iwrk], wavefieldAxis, sourceAxis[iwrk]])
...
class nonlinearPropElasticShotsGpu(Op.Operator):
  ...
  def __init__(self, domain, range, elasticParam, paramP,
    ...
    self.pyOp = pyElastic_iso_float_nl.nonlinearPropElasticShotsGpu(
        elasticParam, paramP.param, sourcesVectorCenterGrid, sourcesVectorXGrid,
...
def nonlinearFwiOpInitFloat(args, client=None):
  ...
  if client:
    ...
    elasticParamFloat = pyDaskVector.DaskVector(client,
                                                vectors=[elasticParamFloat] *
                                                nWrks)
    elasticParamFloatConv = pyDaskVector.DaskVector(
        client, vectors=[elasticParamFloatConv] * nWrks)
...
class nonlinearFwiPropElasticShotsGpu(Op.Operator):
  ...
  def __init__(self, domain, range, sources, paramP, sourcesVectorCenterGrid,
               ...
               receiversVectorCenterGrid, receiversVectorXGrid,
               receiversVectorZGrid, receiversVectorXZGrid):
    #Domain = source wavelet
    #Range = recorded data space
    self.setDomainRange(domain, range)
    #Checking if getCpp is present
    ...
    self.pyOp = pyElastic_iso_float_nl.nonlinearPropElasticShotsGpu(
        domain, paramP.param, sourcesVectorCenterGrid, sourcesVectorXGrid,
        sourcesVectorZGrid, sourcesVectorXZGrid, receiversVectorCenterGrid,
        receiversVectorXGrid, receiversVectorZGrid, receiversVectorXZGrid)
    return
...
def BornOpInitFloat(args, client=None):
  ...
  if client:
    ...
    #Spreading source wavelet
    sourcesSignalsFloat = pyDaskVector.DaskVector(
        client, vectors=[sourcesSignalsFloat] * nWrks).vecDask
    sourcesSignalsVector = client.getClient().map((lambda x: [x]),
                                                  sourcesSignalsFloat,
                                                  pure=False)
    ...
    #Spreading velocity model to workers
    elasticParamHyper = elasticParamFloat.getHyper()
    elasticParamFloatD = pyDaskVector.DaskVector(client,
                                                 vectors=[elasticParamFloat] *
                                                 nWrks)
...
class BornElasticShotsGpu(Op.Operator):
  ...
  def __init__(self, domain, range, elasticParam, paramP, sourcesSignalsVector,
    ...
    self.pyOp = pyElastic_iso_float_born.BornElasticShotsGpu(
        elasticParam, paramP.param, sourcesSignalsVector,
        sourcesVectorCenterGrid, sourcesVectorXGrid, sourcesVectorZGrid,
        sourcesVectorXZGrid, receiversVectorCenterGrid, receiversVectorXGrid,
        receiversVectorZGrid, receiversVectorXZGrid)
    return
