...
def forcing_term_op_init(args):
  ...
  #scale
  scaleOp = Op.scalingOp(
      output, 1 / (centerHyper.getAxis(1).d * centerHyper.getAxis(2).d))
  ...
  priorData = SepVector.getSepVector(SPK_adj.getRange().getHyper(),
                                     storage="dataFloat")
  priorModel = SepVector.getSepVector(SPK_adj.getDomain().getHyper(),
                                      storage="dataFloat")
...
def data_extraction_op_init(args):
  ...
  # Interp operator init
  zCoord, xCoord, centerHyper = SpaceInterpMultiFloat.space_interp_multi_init_rec(
      args)
  ...
  padTruncateRecOp = PadTruncateSourceFloat.pad_truncate_source(
