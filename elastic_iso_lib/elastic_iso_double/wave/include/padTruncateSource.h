...
/*!
 Used for creating a wavefield (z,x,t,w) from a source function (t,w,x,z) or functions
*/
class padTruncateSource : public Operator<double3DReg, double4DReg> {
    ...
    */
		padTruncateSource(const std::shared_ptr<double3DReg> model, const std::shared_ptr<double4DReg> data, std::vector<int> gridPointIndexUnique);

    //! FWD
		/*!
    * this pads from source to wavefield
    */
    void forward(const bool add, const std::shared_ptr<double3DReg> model, std::shared_ptr<double4DReg> data) const;

    //! ADJ
    /*!
    * this truncates from wavefield to source
    */
		void adjoint(const bool add, std::shared_ptr<double3DReg> model, const std::shared_ptr<double4DReg> data) const;
