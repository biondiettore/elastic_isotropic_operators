...
#include <cstring>
waveEquationElasticGpu::waveEquationElasticGpu(const std::shared_ptr<double4DReg> model, const std::shared_ptr<double4DReg> data, std::shared_ptr<SEP::double3DReg> elasticParam, std::shared_ptr<paramObj> par){
...
void waveEquationElasticGpu::forward(const bool add, const std::shared_ptr<double4DReg> model, std::shared_ptr<double4DReg> data) const{
...
void waveEquationElasticGpu::adjoint(const bool add, std::shared_ptr<double4DReg> model, const std::shared_ptr<double4DReg> data) const{
  assert(checkDomainRange(model,data));
...
bool waveEquationElasticGpu::checkGpuMemLimits(float gbLimits){
  ...
  if(_info==1){
    ...
    std::cout << "Global memory to be used per gpu: " << (double)totalSpaceUsedGB/(double)_nGpu << "[GB]\n";
    std::cerr << std::setprecision(2);
    std::cout << "Percentage of memory used per gpu: " << (double)totalSpaceUsedGB/(double)gbLimits*(double)100/(double)_nGpu << "%\n";
