...
class fdParamElastic{
 	...
 	public:
		...
		// Constructor
		fdParamElastic(const std::shared_ptr<double3DReg> elasticParam, const std::shared_ptr<paramObj> par); /** given a parameter file and a velocity model, ensure the dimensions match the prop will be stable and will avoid dispersion.*/
		// Destructor
		~fdParamElastic();
