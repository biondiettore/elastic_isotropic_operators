...
void nonlinearPropElasticGpu::forward(const bool add, const std::shared_ptr<double3DReg> model, std::shared_ptr<double3DReg> data) const {
  ...
  if (!add){
	  ...
	  std::memcpy(dataTemp_vx->getVals(),data->getVals(), chunk*sizeof(double) );
		shift = chunk;
		chunk = _nReceiversIrregZGrid*_fdParamElastic->_nts;
	  std::memcpy(dataTemp_vz->getVals(), data->getVals()+shift, chunk*sizeof(double) );
		shift += chunk;
		chunk = _nReceiversIrregCenterGrid*_fdParamElastic->_nts;
	  std::memcpy(dataTemp_sigmaxx->getVals(), data->getVals()+shift, chunk*sizeof(double) );
		shift += chunk;
		chunk = _nReceiversIrregCenterGrid*_fdParamElastic->_nts;
	  std::memcpy(dataTemp_sigmazz->getVals(), data->getVals()+shift, chunk*sizeof(double) );
		shift += chunk;
		chunk = _nReceiversIrregXZGrid*_fdParamElastic->_nts;
	  std::memcpy(dataTemp_sigmaxz->getVals(), data->getVals()+shift, chunk*sizeof(double) );
  ...
  std::memcpy(modelTemp_vx->getVals(), model->getVals(), chunk*sizeof(double));
	shift = chunk;
	chunk = _nSourcesIrregZGrid*_fdParamElastic->_nts;
  std::memcpy(modelTemp_vz->getVals(), model->getVals()+shift, chunk*sizeof(double));
	shift += chunk;
	chunk = _nSourcesIrregCenterGrid*_fdParamElastic->_nts;
  std::memcpy(modelTemp_sigmaxx->getVals(), model->getVals()+shift, chunk*sizeof(double));
	shift += chunk;
	chunk = _nSourcesIrregCenterGrid*_fdParamElastic->_nts;
  std::memcpy(modelTemp_sigmazz->getVals(), model->getVals()+shift, chunk*sizeof(double));
	shift += chunk;
	chunk = _nSourcesIrregXZGrid*_fdParamElastic->_nts;
  std::memcpy(modelTemp_sigmaxz->getVals(), model->getVals()+shift, chunk*sizeof(double));
  ...
  std::memcpy(data->getVals(), dataTemp_vx->getVals(), chunk*sizeof(double));
	shift += chunk;
	chunk = _nReceiversIrregZGrid*_fdParamElastic->_nts;
  std::memcpy(data->getVals()+shift, dataTemp_vz->getVals(), chunk*sizeof(double));
	shift += chunk;
	chunk = _nReceiversIrregCenterGrid*_fdParamElastic->_nts;
  std::memcpy(data->getVals()+shift, dataTemp_sigmaxx->getVals(), chunk*sizeof(double));
	shift += chunk;
	chunk = _nReceiversIrregCenterGrid*_fdParamElastic->_nts;
  std::memcpy(data->getVals()+shift, dataTemp_sigmazz->getVals(), chunk*sizeof(double));
	shift += chunk;
	chunk = _nReceiversIrregXZGrid*_fdParamElastic->_nts;
  std::memcpy(data->getVals()+shift, dataTemp_sigmaxz->getVals(), chunk*sizeof(double));
...
void nonlinearPropElasticGpu::adjoint(const bool add, std::shared_ptr<double3DReg> model, const std::shared_ptr<double3DReg> data) const {
	...
	std::memcpy(model->getVals(), modelTemp_vx->getVals(), chunk*sizeof(double) );
	shift += chunk;
	chunk = _nSourcesIrregZGrid*_fdParamElastic->_nts;
	std::memcpy(model->getVals()+shift, modelTemp_vz->getVals(), chunk*sizeof(double) );
	shift += chunk;
	chunk = _nSourcesIrregCenterGrid*_fdParamElastic->_nts;
	std::memcpy(model->getVals()+shift, modelTemp_sigmaxx->getVals(), chunk*sizeof(double) );
	shift += chunk;
	chunk = _nSourcesIrregCenterGrid*_fdParamElastic->_nts;
	std::memcpy(model->getVals()+shift, modelTemp_sigmazz->getVals(), chunk*sizeof(double) );
	shift += chunk;
	chunk = _nSourcesIrregXZGrid*_fdParamElastic->_nts;
	std::memcpy(model->getVals()+shift, modelTemp_sigmaxz->getVals(), chunk*sizeof(double));
}
